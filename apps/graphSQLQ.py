...
def add_stats(x):
    ...
    x['cf_khac'] = x['cf_tong'] - x['net_income'] - x['cf_dep'] - x['cf_e_raise'] - x['cf_div'] - x['cf_treasury'] - x[
        'cf_capex']
...
def add_df(ticker):
    ...
    x = pd.merge(x, y, on=['Ticker','year', 'quarter'],how='outer')
    add_ratios(x)
    for i in col3:
        ttm(x, i)
    for i in col1:
    ...
    for i in col2:
        g_func(x, i)
    add_stats(x)
    return x.sort_index(level=['Ticker','year','quarter'])
...
def clean_data(ticker):
    ...
    # statsQ = statsQ.replace(np.nan,0)
...
def asset(dat):
    ...
    asset_bar6 = go.Bar(y=statsQ.loc[:, "lt_inv"], x=statsQ.loc[:, "dates"], name="Công ty LDLK",
                        marker=dict(color=('deeppink')))
