...
def get_secQ(ticker):
    ...
    try:
        ...
        fs.loc[:, 'value'] = fs['value'].astype(float)
        fs.loc[
            (fs['type'] == 91) & (fs['item'] == 'Chi phí lãi vay') & (fs['year'] < 2015), 'item'] = 'Chi phí lãi vay2'
...
layout = html.Div(children=[
    html.Header(children='Graph công ty chứng khoán Quý', className='ml-4',
...
def profit(dat):
    ...
    test1 = go.Bar(y=statsQ.loc[:, "broker_inc"], x=statsQ.loc[:, "dates"], name='LN môi giới',
                   marker=dict(color=('teal')))
    test2 = go.Bar(y=statsQ.loc[:, "margin_inc"], x=statsQ.loc[:, "dates"], name='LN margin',
                   marker=dict(color=('#A4DE02')))
    test3 = go.Bar(y=statsQ.loc[:, "service_inc"], x=statsQ.loc[:, "dates"], name='LN dịch vụ',
                   marker=dict(color=('rgb(200, 0, 0)')))
    test4 = go.Bar(y=statsQ.loc[:, "tudoanh_unrealized_inc"], x=statsQ.loc[:, "dates"],
                   name='LN tự doanh chưa thực hiện',
                   marker=dict(color=('deeppink')))
    test5 = go.Bar(y=statsQ.loc[:, "tudoanh_realized_inc"], x=statsQ.loc[:, "dates"], name='LN tự doanh đã thực hiện',
                   marker=dict(color=('purple')))
    test6 = go.Bar(y=statsQ.loc[:, "fin_inc"], x=statsQ.loc[:, "dates"], name='Lợi nhuận tài chính',
                   marker=dict(color='lavender'))
    test7 = go.Bar(y=statsQ.loc[:, "other_inc"], x=statsQ.loc[:, "dates"], name='Lợi nhuận khác',
                   marker=dict(color='darkgray'))
    test8 = go.Bar(y=-statsQ.loc[:, "admin_exp"], x=statsQ.loc[:, "dates"], name='Chi phí quản lý',
...
def roae(dat):
    statsQ = pd.read_json(dat, orient='split')
    test1 = go.Bar(y=statsQ.loc[:, "core_inc"], x=statsQ.loc[:, "dates"], name='Thu nhập lõi chứng khoán',
                       marker=dict(color=('teal')))
    test2 = go.Bar(y=statsQ.loc[:, "tudoanh_inc"], x=statsQ.loc[:, "dates"], name='Thu nhập tự doanh',
                       marker=dict(color=('deeppink')))
    test3 = go.Bar(y=statsQ.loc[:, "fin_inc"], x=statsQ.loc[:, "dates"], name='Thu nhập tài chính',
                       marker=dict(color=('lavender')))
    test4 = go.Bar(y=statsQ.loc[:, "other_inc2"], x=statsQ.loc[:, "dates"], name='Thu nhập khác',
                       marker=dict(color=('darkgray')))
    test5 = go.Bar(y=-np.abs(statsQ.loc[:, "int_exp"]), x=statsQ.loc[:, "dates"], name='Chi phí lãi vay',
                       marker=dict(color=('orange')))
    test6 = go.Scatter(y=statsQ.loc[:, "net_income"], x=statsQ.loc[:, "dates"], name="Lợi nhuận sau thuế",
...
def asset(dat):
    ...
    asset_bar3 = go.Bar(y=statsQ.loc[:, "bs_st_fvtpl"], x=statsQ.loc[:, "dates"], name="Chứng khoán FVTPL",
                        marker=dict(color=('#A4DE02')))
    asset_bar4 = go.Bar(y=statsQ.loc[:, "bs_afs"], x=statsQ.loc[:, "dates"], name="Chứng khoán AFS",
                        marker=dict(color=('rgb(200, 0, 0)')))
    asset_bar5 = go.Bar(y=statsQ.loc[:, "bs_htm"], x=statsQ.loc[:, "dates"], name="Chứng khoán HTM",
                        marker=dict(color=('orange')))
...
def equity(dat):
    ...
    asset_bar2 = go.Bar(y=statsQ.loc[:, "re"], x=statsQ.loc[:, "dates"], name="LN chưa phân phối",
                        marker=dict(color=('#A4DE02')))
    asset_bar3 = go.Bar(y=statsQ.loc[:, "quy_duphong"], x=statsQ.loc[:, "dates"], name="Quỹ dự phòng")
    ...
    asset_bar6 = go.Bar(y=statsQ.loc[:, "lt_debt"], x=statsQ.loc[:, "dates"], name='Vay dài hạn',
                        marker=dict(color=('rgb(200, 0, 0)')))
    asset_bar7 = go.Bar(y=statsQ.loc[:, "lia_bond"], x=statsQ.loc[:, "dates"], name="Trái phiếu",
                        marker=dict(color=('yellow')))
    asset_bar8 = go.Bar(y=statsQ.loc[:, "other_lia"], x=statsQ.loc[:, "dates"], name="Nợ khác",
                        marker=dict(color=('darkgray')))
...
def cf(dat):
    ...
    bar4 = go.Bar(y=statsQ.loc[:, "cf_e_raise"], x=statsQ.loc[:, "dates"], name="Tăng vốn",
                  marker=dict(color=('deeppink')))
    bar5 = go.Bar(y=statsQ.loc[:, "cf_treasury"], x=statsQ.loc[:, "dates"], name="CP quỹ",
                  marker=dict(color=('yellow')))
    bar6 = go.Bar(y=statsQ.loc[:, "other_cf"], x=statsQ.loc[:, "dates"], name="CF khác",
                  marker=dict(color=('darkgray')))
