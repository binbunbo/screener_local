...
df = pd.read_pickle('D:/Data/DataSQL/df_merge')
...
def add_dfY(ticker):
    ...
    x = pd.merge(x, y, on=['Ticker', 'year'],how='outer')
    add_ratios(x)
    for i in col1:
        margin_func(x,i)
    for i in col3:
        g_func(x, i)
    add_stats(x)
    return x.sort_index(level=['Ticker','year'])
...
layout = html.Div(children=[
    ...
    html.Div(id='intermediate-valueSQL', style={'display': 'none'}),
    html.Br(),
    html.Br(),
    dbc.Container(id='price-thuong-SQLY',className='six columns',
                 style={'width': '30%', 'display': 'inline-block','margin-left':'5px'}),
    html.Div([
        html.Div([dcc.Graph(id='output-graph11SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
        html.Div([dcc.Graph(id='output-graph12SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
    ], className='row'),
    html.Div([
        html.Div([dcc.Graph(id='output-graph13SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
        html.Div([dcc.Graph(id='output-graph14SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
    ], className='row'),
    html.Div([
        html.Div([dcc.Graph(id='output-graph15SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
        html.Div([dcc.Graph(id='output-graph16SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
    ], className='row'),
    html.Div([
        html.Div([dcc.Graph(id='output-graph17SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
        html.Div([dcc.Graph(id='output-graph18SQL')], className='six columns',
                 style={'width': '50%', 'display': 'inline-block'}),
    ], className='row')
])
@app.callback(Output('price-thuong-SQLY', 'children'),
...
def display_price(ticker):
    ...
    return alert
@app.callback(Output('intermediate-valueSQL', 'children'), [Input(component_id='input', component_property='value')])
...
@app.callback(
    Output(component_id='output-graph11SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def profit(dat):
...
@app.callback(
    Output(component_id='output-graph12SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def roae(dat):
    statsY = pd.read_json(dat, orient='split')
    ...
    test3 = go.Bar(y=statsY.loc[:, 'op'], x=statsY.loc[:, 'year'], name='LN cốt lõi',
                   marker=dict(color=('teal')))
    test4 = go.Bar(y=-np.abs(statsY.loc[:, 'int_exp']), x=statsY.loc[:, 'year'], name='Chi phí lãi vay',
...
@app.callback(
    Output(component_id='output-graph13SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def asset(dat):
    statsY = pd.read_json(dat, orient='split')
    ...
    asset_bar5 = go.Bar(y=statsY.loc[:, "cip"], x=statsY.loc[:, "year"], name="XDCB",
                        marker=dict(color=('goldenrod')))
    asset_bar6 = go.Bar(y=statsY.loc[:, "lt_inv"], x=statsY.loc[:, "year"], name="Công ty LDLK",
                        marker=dict(color=('deeppink')))
...
@app.callback(
    Output(component_id='output-graph14SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def equity(dat):
    statsY = pd.read_json(dat, orient='split')
...
@app.callback(
    Output(component_id='output-graph16SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def cf(dat):
    statsY = pd.read_json(dat, orient='split')
...
@app.callback(
    Output(component_id='output-graph15SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def growth(dat):
    statsY = pd.read_json(dat, orient='split')
    ...
    return fig.update_layout(template='plotly_dark', title_x=0.5,
                             ...
                             paper_bgcolor='rgba(0, 0, 0, 0)')
@app.callback(
    Output(component_id='output-graph17SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def pe(dat):
    statsY = pd.read_json(dat, orient='split')
...
@app.callback(
    Output(component_id='output-graph18SQL', component_property='figure'),
    [Input(component_id='intermediate-valueSQL', component_property='children')]
)
def pb(dat):
    statsY = pd.read_json(dat, orient='split')
